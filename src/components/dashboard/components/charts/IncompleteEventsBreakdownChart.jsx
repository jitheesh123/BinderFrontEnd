...
class EventBreakdownChart extends Component {
    ...
    componentDidUpdate = async (prevProps, prevState) => {
        if (!_.isEqual(prevProps.IncompletedEventBreakdownData, this.props.IncompletedEventBreakdownData)) {
            await this.renderChartData();
        }
    ...
    renderChartData = async () => {
        const { IncompletedEventBreakdownData, toggleSmartReportModal } = this.props;
        let tempData = graphDummyData.IncompletedEventBreakdownData.map(item => {
            return { label: this.replaceSpaceWithNewLine(item.status), name: item.status, y: item.count, color: this.getColor(item.status) };
        });
        let total = 0;
        if (IncompletedEventBreakdownData.length) {
            tempData = IncompletedEventBreakdownData.map(item => {
        ...
        dataSource = {
            chart: {
                ...
                options3d: {
                    enabled: chartType == "doughnut3d" || chartType == "3D" ? true : false,
                    alpha: 45,
                    beta: 0
                },
                style: {
            ...
            plotOptions: {
                ...
                series: {
                    ...
                    events: {
                        click: event => {
                            toggleSmartReportModal("incomplete_events_breakdown", event.point.name.toLowerCase().replace(/ /g, "_"));
                        }
                    },
                    startAngle: 90,
                    dataLabels: {
    ...
    convertToImage = imgType => {
        ...
        let name = `Incompleted_Events_Breakdown-${user}-${moment().format("MM_DD_YYYY HH_mm_ss")}`;
        if (imgType == "jpeg") {
            htmlToImage.toJpeg(document.getElementById("chartIncomEvBr"), { quality: 0.95 }).then(function (dataUrl) {
            ...
            htmlToImage.toPng(document.getElementById("chartIncomEvBr"), { quality: 0.95 }).then(function (dataUrl) {
                var link = document.createElement("a");
    ...
    renderClass = () => {
        ...
        if (isFullScreen) {
            if (fullScreenChart === "incomplete_events_breakdown") {
                return "col-12";
            } else {
                return "d-none";
            }
    ...
    render() {
        ...
        const { fullViewHandler, isFullScreen, IncompletedEventBreakdownData } = this.props;
        return (
            <div className={`graph-bx col-xl-4 col-lg-4 col-md-6 ${this.renderClass()}`}>
                <div className="graph-hd">
                    <strong>Incomplete Activity events breakdown</strong>
                    ...
                    <div className="btn-grp btn-grp-top ml-1 p-0">
                        <button className="btn" onClick={() => fullViewHandler("incomplete_events_breakdown")}>
                            <img src="images/restore.svg" alt="" />
                        </button>
                    </div>
                </div>
                <div className={`graph-content ${!IncompletedEventBreakdownData.length ? "no-data-sctn" : ""}`} data-descr="No Activity To Report">
                    <div className="g-box" id="chartIncomEvBr">
