...
class EventBreakdownChart extends Component {
    ...
    renderChartData = async () => {
        const { eventBreakdownData, toggleSmartReportModal } = this.props;
        ...
        dataSource = {
            ...
            plotOptions: {
                ...
                series: {
                    ...
                    events: {
                        click: event => {
                            toggleSmartReportModal("event_breakdown", event.point.name.toLowerCase().replace(/ /g, "_"));
                        }
                    },
                    startAngle: 90,
                    dataLabels: {
    ...
    convertToImage = imgType => {
        ...
        let name = `Event_Breakdown-${user}-${moment().format("MM_DD_YYYY HH_mm_ss")}`;
        if (imgType == "jpeg") {
    ...
    renderClass = () => {
        ...
        if (isFullScreen) {
            if (fullScreenChart === "event_breakdown") {
                return "col-12";
            } else {
                return "d-none";
            }
    ...
    render() {
        ...
        const { fullViewHandler, isFullScreen, eventBreakdownData } = this.props;
        return (
            <div className={`graph-bx col-xl-4 col-lg-4 col-md-6 ${this.renderClass()}`}>
                <div className="graph-hd">
                    <strong>Activity Event Breakdown</strong>
                    ...
                    <div className="btn-grp btn-grp-top ml-1 p-0">
                        <button className="btn" onClick={() => fullViewHandler("event_breakdown")}>
                ...
                <div className={`graph-content ${!eventBreakdownData.length ? "no-data-sctn" : ""}`} data-descr="No Activity To Report">
                    <div className="g-box" id="chartItem">
