...
class ActivityCalendarTableCalendar extends Component {
    constructor(props) {
        super(props);
        this.state = {
            calendarColumns: moment.monthsShort(),
    ...
    componentDidMount = async () => {
        window.addEventListener("click", e => {
            let trailingCalendarTableHeaderRow = document.getElementById("trailingCalendarTableHeaderRow");
            if (trailingCalendarTableHeaderRow && !trailingCalendarTableHeaderRow.contains(e.target) && !e.target.hasAttribute("is-dropdown-item")) {
                this.setState({
                    selectedFieldForCommonSearch: null
                });
            }
            ...
            if (!_.isEmpty(this.props.commonFilter)) {
                ...
                Object.keys(this.props.commonFilter).map(fil => {
                    if (
                        ...
                        this.props.commonFilter[fil].length == this.state.filterList.length
                    ) {
    ...
    setCalendarColumns = (type = true) => {
        ...
        moment.monthsShort().map((month, i) => {
            if (currentMonth >= i) {
                currentTemp.push({ year: currentYear, month: month, index: i, class: "th-active" });
            } else {
                prevTemp.push({ year: previousYear, month: month, index: i, class: "" });
            }
        });
    ...
    monthIsNA = (na_start, na_end, month, year) => {
        let na_start_month = new Date(na_start).getMonth();
        let na_end_month = new Date(na_end).getMonth();
        let na_end_year = new Date(na_end).getFullYear();
        let na_start_year = new Date(na_start).getFullYear();

        if (month >= na_start_month && month <= na_end_month && year <= na_end_year && year >= na_start_year) {
            return true;
        }
        return false;
    };
    ...
    findWorstCaseDateObject = (item, monthIndex, year) => {
        ...
        const getScheduleDates = schedule_dates.schedules
            ...
            ?.dates.filter(dateItem => new Date(dateItem.date).getMonth() === monthIndex && new Date(dateItem.date).getFullYear() === year);
        let dateList =
            item.dates && item.dates.length
                ? item.dates.filter(dateItem => new Date(dateItem.date).getMonth() === monthIndex && new Date(dateItem.date).getFullYear() === year)
                : [];
    ...
    renderMonthlySchedule = item => {
        ...
        const { calendarColumns } = this.state;
        let tableContentHtml = calendarColumns.map((monthItem, i) => {
            let currentYear = new Date().getFullYear();
            if (monthItem.year === currentYear) {
                if (item.na_start && item.na_end) {
                    if (this.monthIsNA(item.na_start, item.na_end, monthItem.index, monthItem.year)) {
                        return (
                            <td className="date-wrapper cursor-pointer">
                                <div
                                    className="icon-date-wrapper na-container"
                                    onClick={() => handleNAClick(item.id, monthItem.index, monthItem.year)}
                                >
                                    <span>NA</span>
                                </div>
                            </td>
                ...
                if (item.previous_na && this.monthIsNA(item.previous_na.na_start, item.previous_na.na_end, monthItem.index, item.previous_na.year)) {
                    return (
                        <td className="date-wrapper cursor-pointer">
                            <div
                                className="icon-date-wrapper na-container"
                                onClick={() => handleNAClick(item.id, monthItem.index, item.previous_na.year)}
                            >
                                <span>NA</span>
                            </div>
                        </td>
            ...
            let dateObjOfMonth = this.findDatesOfMonth(item.dates, monthItem.index, monthItem.year);
            if (dateObjOfMonth) {
                if (dateObjOfMonth.day || dateObjOfMonth.week) {
                    let worstCaseDateObject = this.findWorstCaseDateObject(item, monthItem.index, monthItem.year);
                    if (worstCaseDateObject) {
                        return (
                            <td
                ...
                return (
                    <td
                        ...
                        onClick={() => showCreateActivityEventSchedule(item.id, monthItem.index, monthItem.year, "add")}
                    ></td>
                );
            }
        });
    ...
    renderQuaterlySchedule = item => {
        ...
        const { handleActivityEventClick, showCreateActivityEventSchedule, handleNAClick, calendarView } = this.props;
        const { calendarColumns } = this.state;
        let quaterClass = "";
        let quaterMainClass = "";
        let currentYear = new Date().getFullYear();
        let currentMonth = new Date().getMonth();
        let tableContentHtml = calendarColumns.map((monthItem, i) => {
            let uniqueClass = `${currentYear === monthItem.year ? "_current" : "_prev"}_${item.id}`;
            if (monthItem.index < 3) {
                quaterMainClass = `quater_one${uniqueClass}`;
                quaterClass = `${quaterMainClass} quater_one`;
                if (calendarView) {
                    if (monthItem.index === 0) {
                        quaterClass = `${quaterClass} last`;
                    }
                } else {
                    if (monthItem.index === 2) {
                        quaterClass = `${quaterClass} last`;
                    }
                }
            } else if (monthItem.index < 6) {
                quaterMainClass = `quater_two${uniqueClass}`;
                ...
                if (calendarView) {
                    ...
                    if (monthItem.index === 5) {
                        quaterClass = `${quaterClass} last`;
                    }
                }
            } else if (monthItem.index < 9) {
                quaterMainClass = `quater_three${uniqueClass}`;
                quaterClass = `${quaterMainClass} quater_three`;
                if (calendarView) {
                    if (monthItem.index === 6) {
                        quaterClass = `${quaterClass} last`;
                    }
                } else {
                    if (monthItem.index === 8) {
                        quaterClass = `${quaterClass} last`;
                    }
                }
            } else {
                quaterMainClass = `quater_four${uniqueClass}`;
                quaterClass = `${quaterMainClass} quater_four`;
                if (calendarView) {
                    if (monthItem.index === 9) {
                        quaterClass = `${quaterClass} last`;
                    }
                } else {
                    if (monthItem.index === 11) {
                        quaterClass = `${quaterClass} last`;
                    }
                }
            }

            // SETTING LAST MONTH IN A QUATER FOR CURRENT YEAR
            if (!calendarView && currentYear === monthItem.year && currentMonth === monthItem.index) {
                quaterClass = quaterClass + " last";
            }
            ...
            if (monthItem.year === currentYear) {
                if (item.na_start && item.na_end) {
                    if (this.monthIsNA(item.na_start, item.na_end, monthItem.index, monthItem.year)) {
                        return (
                            <td className={`${quaterClass} date-wrapper cursor-pointer`}>
                                <div
                                    className="icon-date-wrapper na-container"
                                    onClick={() => handleNAClick(item.id, monthItem.index, monthItem.year)}
                                >
            ...
            if (dateObjOfMonth) {
                ...
                // CLICK FOR EMPTY EVENT CELLS IN QUATER
                if (monthItem.index % 3 === 0) {
                    let dateObjOfMonthNext = this.findDatesOfMonth(item.dates, monthItem.index + 1, monthItem.year);
                    let dateObjOfMonthSuperNext = this.findDatesOfMonth(item.dates, monthItem.index + 2, monthItem.year);
                ...
                } else if (monthItem.index % 3 === 1) {
                    let dateObjOfMonthPrev = this.findDatesOfMonth(item.dates, monthItem.index - 1, monthItem.year);
                    let dateObjOfMonthNext = this.findDatesOfMonth(item.dates, monthItem.index + 1, monthItem.year);
                ...
                } else if (monthItem.index % 3 === 2) {
                    let dateObjOfMonthPrev = this.findDatesOfMonth(item.dates, monthItem.index - 1, monthItem.year);
                    let dateObjOfMonthSuperPrev = this.findDatesOfMonth(item.dates, monthItem.index - 2, monthItem.year);
                    if (dateObjOfMonthPrev) {
                        return (
                            <td
                                className={`cursor-pointer ${quaterClass}`}
    ...
    hasBorder = dateObjOfMonth => {
        let today = new Date();
        if (
            new Date(dateObjOfMonth.threshold_start).setHours(0, 0, 0, 0) <= today.setHours(0, 0, 0, 0) &&
            new Date(dateObjOfMonth.threshold_end).setHours(0, 0, 0, 0) >= today.setHours(0, 0, 0, 0)
        ) {
            return true;
        }
        return false;
    ...
    findDatesOfMonth = (dates, month, year) => {
        let dateOfMonth = null;
        dates.map((dateObj, i) => {
            if (new Date(dateObj.date).getMonth() === month && new Date(dateObj.date).getFullYear() === year) {
                dateOfMonth = dateObj;
            }
        });
        return dateOfMonth;
                    ...
                    <div className={`file-calendar-cover ${isCalendarExpanded ? "cal-tab-active" : ""}`}>
                        <div className="calendar-tab-cover" id="calendar-table-container" onScroll={e => this.OnScroll(e)}>
                            <table className="calendar-table">
                                <thead>
                                    <tr id="trailingCalendarTableHeaderRow">
                                        ...
                                        <th
                                            className={`calendar-acticity-column3 min-width-110 drop-fil drop-content ${
                                        ...
                                        {calendarColumns.map((item, i) => (
                                            <th
                                                className={`${this.checkHasWildCardFiltersForDates(item.month) ? "active-head" : ""} ${
                                                    item.class
                                                } calendar-acticity-column${i + 4}`}
                                                key={i}
                                            >
                                                <span>
                                                    {item.month} {item.year && item.year !== "undefined" ? item.year.toString().slice(-2) : null}
                                                </span>
                                            </th>
                                        ))}
                                    </tr>
                                ...
                                <tbody>
                                    {schedules.length ? (
                                        <>
                                            <ReactTooltip id="trailing_calendar" effect="solid" />
                                            ...
                                            {schedules.map((item, i) => (
                                                <tr key={i}>
                                                    {selectedLogbook.has_asset === "yes" ? (
                                                        ...
                                                        data-for="trailing_calendar"
                                                        data-tip={item.asset.name}
                                                        data-place="top">
                                                            <Highlighter
                                                                searchWords={
